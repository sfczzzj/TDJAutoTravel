...
def FindAndClick(x1, y1, x2, y2, image, thld):
	res = None
	res = FindImageInRect(x1, y1, x2, y2, image, thld)
	if (res != None):
		AntiCheck.RangeClick(res[0], res[1], 5)
		return 1
	return None

def AppImageShot(x1, y1, x2, y2):
	src = pyautogui.screenshot(region=[RX(x1), RY(y1), RX(x2) - RX(x1), RY(y2) - RY(y1)])
	
	return cv2.cvtColor(np.asarray(src), cv2.COLOR_RGB2BGR)